...
export default class userController {
    async signupUser(req, res) {
        try {
            ...
            const result = await new userManager().signUp({name, email, password: hashedPassword, type, address, phone});
            if(!result) {
    ...
    async updateUser(req, res) {
        try {
            ...
            const result = await new userManager().updateUser(id, { name, email, address, phone });
            if(!result.success) {
                res.status(500).send(result);
            } else {
                res.status(200).send(result);
